...
// Shows Router → Analyst → Comm (per the deck) plus the alert feed and actions.
...
function PipelineCard({ p, t, engine, model }: { p: PipelineTrace; t: Theme; engine: string; model: string }) {
  const sevColor =
    p.router.severity === "critical" || p.router.severity === "high" ? t.red : p.router.severity === "med" ? t.amber : p.router.severity === "low" ? t.green : t.dim;
...
export default function Live({
  s, t, trip, tick, loading, engine, model, pipeline, alerts, onTick, onReset, maxTicks,
}: {
  s: Schedule;
  t: Theme;
  trip: Trip;
      ...
      <div style={{ background: `linear-gradient(150deg, ${t.surface2}, ${t.surface})`, border: `1px solid ${s.status.color}44`, borderRadius: t.radius, padding: t.pad }}>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 12 }}>
          ...
          <span style={{ fontFamily: t.body, fontSize: 13, color: t.dim }}>
            ETA {trip.destination.code} · {s.slip <= 0 ? "on schedule" : `+${fmtDur(s.slip)} vs plan`}
          </span>
        </div>
      </div>
          ...
          {alerts.length === 0 && (
            <div style={{ padding: 22, textAlign: "center", borderRadius: t.radius, border: `1px dashed ${t.line}`, background: t.surface }}>
              ...
              <div style={{ fontFamily: t.body, fontSize: 13, color: t.dim, marginTop: 8 }}>
                Agent armed. Run a tick to let Gemma sweep the feeds and decide what affects your arrival.
              </div>
            </div>
          )}
          {alerts.map((e) => {
