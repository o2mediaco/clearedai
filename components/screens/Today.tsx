...
// Responsive: single column on mobile, 2-column grid on desktop (wide).
...
function HeroEta({ s, t, trip, big }: { s: Schedule; t: Theme; trip: Trip; big: boolean }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontFamily: t.mono, fontSize: 12, letterSpacing: 1.5, textTransform: "uppercase", color: t.faint, marginTop: 16 }}>Arriving {trip.destination.city}</div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginTop: 6, flexWrap: "wrap" }}>
        <span style={{ fontFamily: t.display, fontWeight: 600, fontSize: big ? 60 : 46, lineHeight: 0.95, color: t.text, letterSpacing: -1 }}>{fmtTime(s.arriveUtc, tz)}</span>
      ...
      <div style={{ display: "flex", gap: 28, marginTop: 18 }}>
        <div>
          ...
          <div style={{ fontFamily: t.display, fontSize: 22, fontWeight: 600, color: s.status.color, marginTop: 3 }}>{s.slip <= 0 ? "On schedule" : `+${fmtDur(s.slip)}`}</div>
        </div>
        <div style={{ width: 1, background: t.line }} />
        <div>
          <div style={{ fontFamily: t.mono, fontSize: 11, color: t.faint, letterSpacing: 1, textTransform: "uppercase" }}>Leave by</div>
          <div style={{ fontFamily: t.display, fontSize: 22, fontWeight: 600, color: t.text, marginTop: 3 }}>{fmtTime(s.leaveByUtc, trip.origin.tz)}</div>
        </div>
      </div>
    </div>
  );
...
export default function Today({ s, t, nowUtc, go, trip, wide }: { s: Schedule; t: Theme; nowUtc: number; go: (tab: Tab) => void; trip: Trip; wide: boolean }) {
  ...
  const heroCard = (
    <div style={{ background: `linear-gradient(160deg, ${t.surface2}, ${t.surface})`, border: `1px solid ${t.line}`, borderRadius: t.radius + 4, padding: wide ? 28 : t.pad + 4, height: "100%" }}>
      <HeroEta s={s} t={t} trip={trip} big={wide} />
    </div>
  );
    ...
    <Card t={t} style={{ height: "100%" }}>
      ...
      <div style={{ display: "flex", gap: 13, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontFamily: t.mono, fontSize: 12.5, color: t.text, marginTop: 8 }}>ETA {fmtTime(s.arriveUtc, trip.destination.tz)} <span style={{ color: t.faint }}>· {fmtDate(s.arriveUtc, trip.destination.tz)}</span></div>
        </div>
      </div>
    </Card>
  );
  ...
  if (wide) {
    return (
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: t.gap, alignItems: "start" }}>
        ...
        <div style={{ gridColumn: "1 / -1" }}>{heroCard}</div>
